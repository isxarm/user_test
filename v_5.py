...
"selection-background-color: rgb(255, 255, 255);")
        ...
        self.speed_direction_layout = QtWidgets.QVBoxLayout(self.speed_direction_page)
        self.speed_direction_layout.setContentsMargins(30, 30, 30, 30)
        self.speed_direction_layout.setSpacing(20)

        # Digital Speed Display
        self.speed_display_frame = QtWidgets.QFrame(self.speed_direction_page)
        self.speed_display_frame.setStyleSheet("""
            QFrame {
        ...
        """)
        self.speed_display_frame.setMinimumHeight(120)
        self.speed_display_layout = QtWidgets.QHBoxLayout(self.speed_display_frame)
        self.speed_display_layout.setContentsMargins(30, 20, 30, 20)

        # Digital Number
        self.label_speed_value = QtWidgets.QLabel(self.speed_display_frame)
        self.label_speed_value.setText("0.00")
        self.label_speed_value.setFont(QtGui.QFont('DS-Digital', 60, QtGui.QFont.Bold))
        self.label_speed_value.setStyleSheet("color: #00ffff;")
        self.speed_display_layout.addWidget(self.label_speed_value)

        # หน่วย
        self.label_speed_unit = QtWidgets.QLabel(self.speed_display_frame)
        self.label_speed_unit.setText("km/h")
        self.label_speed_unit.setFont(QtGui.QFont('Arial', 28, QtGui.QFont.Bold))
        self.label_speed_unit.setStyleSheet("color: #00ffff; margin-left: 10px;")
        self.speed_display_layout.addWidget(self.label_speed_unit)

        self.speed_direction_layout.addWidget(self.speed_display_frame, 0, QtCore.Qt.AlignHCenter)

        # Direction Frame
        self.direction_frame = QtWidgets.QFrame(self.speed_direction_page)
        self.direction_frame.setStyleSheet("""
            QFrame {
        ...
        """)
        self.direction_frame.setMinimumHeight(80)
        self.direction_layout = QtWidgets.QHBoxLayout(self.direction_frame)
        self.direction_layout.setContentsMargins(30, 10, 30, 10)

        # Direction Icon - ใช้ไอคอนที่ไม่มีอยู่จริงให้แสดงข้อความแทน
        self.label_direction_icon = QtWidgets.QLabel(self.direction_frame)
        self.label_direction_icon.setText("→")  # ใช้ลูกศรแทนรูปภาพ
        self.label_direction_icon.setFont(QtGui.QFont('Arial', 32, QtGui.QFont.Bold))
        self.label_direction_icon.setStyleSheet("color: #00ff99;")
        self.direction_layout.addWidget(self.label_direction_icon)

        # Direction Text
        self.label_direction_text = QtWidgets.QLabel(self.direction_frame)
        self.label_direction_text.setText("FORWARD")
        self.label_direction_text.setFont(QtGui.QFont('Arial', 32, QtGui.QFont.Bold))
        self.label_direction_text.setStyleSheet("color: #00ff99; margin-left: 20px;")
        self.direction_layout.addWidget(self.label_direction_text)

        self.speed_direction_layout.addWidget(self.direction_frame, 0, QtCore.Qt.AlignHCenter)
        ...
        self.speed_direction_layout.addWidget(self.label_speed_dir_title, 0, QtCore.Qt.AlignHCenter)

        # Current Speed Label
        self.label_current_speed = QtWidgets.QLabel(self.speed_direction_page)
        self.label_current_speed.setFont(QtGui.QFont('Arial', 22))
        self.label_current_speed.setStyleSheet("color: rgb(0, 255, 255);")
        self.label_current_speed.setText("Current Speed: -- m/s")
        self.speed_direction_layout.addWidget(self.label_current_speed)

        # Current Direction Label
        self.label_current_direction = QtWidgets.QLabel(self.speed_direction_page)
        self.label_current_direction.setFont(QtGui.QFont('Arial', 22))
        self.label_current_direction.setStyleSheet("color: rgb(0, 255, 255);")
        self.label_current_direction.setText("Current Direction: --")
        self.speed_direction_layout.addWidget(self.label_current_direction)
        ...
        self.label_history.setText("History (Speed/Direction):")
        self.speed_direction_layout.addWidget(self.label_history)
        ...
        """)
        self.speed_direction_layout.addWidget(self.history_list)
...
# 4. เพิ่มเข้า list
        ...
        # เพิ่มเข้า list ของ MainWindow ด้วย
        self.menu_buttons.append(speed_btn)
        self.menu_labels.append(speed_lbl)
    ...
    def handle_menu_click(self, index):
        ...
        if index < 0 or index >= len(self.menu_containers):
            return
        ...
        active_style_button = normal_style_button  # ใช้ style เดียวกัน
        ...
        for container, button, label in zip(self.menu_containers, self.menu_buttons, self.menu_labels):
            if container and button and label:  # ตรวจสอบว่าไม่ใช่ None
                container.setStyleSheet(normal_style_container)
                button.setStyleSheet(normal_style_button)
                label.setStyleSheet(normal_style_label)
        
        # Set active style for clicked button and its label
        if (index < len(self.menu_containers) and 
            index < len(self.menu_buttons) and 
            index < len(self.menu_labels)):
    ...
    def update_speed_direction_details(self, speed_mps, direction):
        speed_kmh = speed_mps * 3.6
        self.ui.label_speed_value.setText(f"{speed_kmh:.2f}")
        if direction == "FORWARD":
            self.ui.label_direction_text.setText("→ FORWARD")
            self.ui.label_direction_text.setStyleSheet("color: #00ff99; font-size: 32px; margin-left: 20px;")
        else:
            self.ui.label_direction_text.setText("← REVERSE")
            self.ui.label_direction_text.setStyleSheet("color: #ff5555; font-size: 32px; margin-left: 20px;")
        self.ui.history_list.addItem(f"{speed_kmh:.2f} km/h, {direction}")
