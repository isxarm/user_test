...
from PyQt5.QtCore import QPropertyAnimation, QEasingCurve, Qt, QTimer, QSize,QMetaObject, pyqtSlot, Q_ARG
...
import threading
from knob_reader import KnobReader
...
"border:None;}")
       ...
       # font.setWeight(300)
        ...
        self.gauge_widget1 = GW(self.gauge1, "VOLTAGE")
        ...
        self.gauge_widget2 = GW(self.gauge2, "CURRENT")
        ...
        self.gauge_widget3 = GW(self.gague3, "FREQUENCY")
        ...
        # Create gauge widget
        self.gauge_widget4 = GW(self.gauge4, "RPM")
...
"selection-background-color: rgb(255, 255, 255);")
        ...
        # Add knob control display to central widget (instead of right_2)
        self.knob_container = QtWidgets.QFrame(self.centralwidget)
        self.knob_container.setGeometry(1050, 20, 200, 200)  # Position in top-right corner
        self.knob_container.setStyleSheet("background: transparent;")
        self.knob_container.raise_()  # Ensure it stays on top
        
        # Create circular button for knob control
        self.knob_button = CircularButton(self.knob_container)
        self.knob_button.setGeometry(70, 70, 60, 60)  # Center position
        
        # Create popup frame for detailed display
        self.knob_popup = QtWidgets.QFrame(self.knob_container)
        self.knob_popup.setGeometry(0, 0, 200, 200)
        self.knob_popup.setStyleSheet("""
            QFrame {
                background-color: rgba(8, 43, 55, 200);
                border-radius: 20px;
                border: 2px solid rgb(0, 255, 255);
            }
        """)
        self.knob_popup.hide()
        self.knob_popup.raise_()  # Ensure popup stays on top
        
        # Add speed and direction labels to popup
        self.speed_label = QtWidgets.QLabel(self.knob_popup)
        self.speed_label.setGeometry(20, 40, 160, 40)
        ...
        self.direction_label = QtWidgets.QLabel(self.knob_popup)
        self.direction_label.setGeometry(20, 90, 160, 40)
        ...
        # Setup animations
        self.popup_animation = QPropertyAnimation(self.knob_popup, b"geometry")
        self.popup_animation.setDuration(300)
        self.popup_animation.setEasingCurve(QEasingCurve.OutBack)
        
        self.button_animation = QPropertyAnimation(self.knob_button, b"geometry")
        self.button_animation.setDuration(300)
        self.button_animation.setEasingCurve(QEasingCurve.OutBack)
        
        # Initialize knob reader
        self.knob_reader = KnobReader()
        self.last_speed = 0
        self.last_direction = "STOP"
        self.is_popup_visible = False
        
        # Setup timer for knob updates
        self.knob_timer = QTimer()
        self.knob_timer.timeout.connect(self.update_knob_display)
        self.knob_timer.start(100)  # Update every 100ms
    ...
    def retranslateUi(self, HMI):
        ...
        self.label.setText(_translate("HMI", "MECHINE CONTROL"))
    ...
    def add_plot(self):
        ...
        self.timer.start(500)  # Update every 100ms
    ...
    def update_gauge_display1(self, parameter, unit, range_values):
        # Update current parameter for gauge 1
        self.current_params['gauge1'] = parameter
        # Update gauge widget with new parameter
        self.ui.gauge_widget1.set_label(self.param_config[parameter]['label'])
        self.ui.gauge_widget1.unit = self.param_config[parameter]['unit']
        self.ui.gauge_widget1.min_value = self.param_config[parameter]['range'][0]
        self.ui.gauge_widget1.max_value = self.param_config[parameter]['range'][1]
        self.ui.gauge_widget1.draw_gauge()
    ...
    def update_gauge_display2(self, parameter, unit, range_values):
        # Update current parameter for gauge 2
        self.current_params['gauge2'] = parameter
        # Update gauge widget with new parameter
        self.ui.gauge_widget2.set_label(self.param_config[parameter]['label'])
        self.ui.gauge_widget2.unit = self.param_config[parameter]['unit']
        self.ui.gauge_widget2.min_value = self.param_config[parameter]['range'][0]
        self.ui.gauge_widget2.max_value = self.param_config[parameter]['range'][1]
        self.ui.gauge_widget2.draw_gauge()
    ...
    def update_gauge_display3(self, parameter, unit, range_values):
        # Update current parameter for gauge 3
        self.current_params['gauge3'] = parameter
        # Update gauge widget with new parameter
        self.ui.gauge_widget3.set_label(self.param_config[parameter]['label'])
        self.ui.gauge_widget3.unit = self.param_config[parameter]['unit']
        self.ui.gauge_widget3.min_value = self.param_config[parameter]['range'][0]
        self.ui.gauge_widget3.max_value = self.param_config[parameter]['range'][1]
        self.ui.gauge_widget3.draw_gauge()
    ...
    def update_gauge_display4(self, parameter, unit, range_values):
        # Update current parameter for gauge 4
        self.current_params['gauge4'] = parameter
        # Update gauge widget with new parameter
        self.ui.gauge_widget4.set_label(self.param_config[parameter]['label'])
        self.ui.gauge_widget4.unit = self.param_config[parameter]['unit']
        self.ui.gauge_widget4.min_value = self.param_config[parameter]['range'][0]
        self.ui.gauge_widget4.max_value = self.param_config[parameter]['range'][1]
        self.ui.gauge_widget4.draw_gauge()
    ...
    def update_cpu_usage(self):
        try:
            # Get CPU usage per core for Raspberry Pi
            cpu_percent = psutil.cpu_percent(interval=None, percpu=True)

            # Get CPU frequency for Raspberry Pi
            with open('/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq', 'r') as f:
                current_freq = float(f.read().strip()) / 1000  # Convert to MHz

            # Get CPU temperature for Raspberry Pi
            with open('/sys/class/thermal/thermal_zone0/temp', 'r') as f:
                temp = float(f.read().strip()) / 1000  # Convert to Celsius

            # Update gauges and info labels
            self.core1_gauge.set_speed(cpu_percent[0])
            self.core1_info.setText(f"Temp: {temp:.1f}°C\n\nClock: {current_freq:.0f} MHz")

            self.core2_gauge.set_speed(cpu_percent[1])
            self.core2_info.setText(f"Temp: {temp:.1f}°C\n\nClock: {current_freq:.0f} MHz")

            self.core3_gauge.set_speed(cpu_percent[2])
            self.core3_info.setText(f"Temp: {temp:.1f}°C\n\nClock: {current_freq:.0f} MHz")

            self.core4_gauge.set_speed(cpu_percent[3])
            self.core4_info.setText(f"Temp: {temp:.1f}°C\n\nClock: {current_freq:.0f} MHz")
            ...
            try:
                ...
                # Update gauges and info labels with psutil data
                self.core1_gauge.set_speed(cpu_percent[0])
                self.core1_info.setText(f"Temp: {temp:.1f}°C\n\nClock: {freq:.0f} MHz")
    ...
    def update_knob_display(self):
        try:
            # Get current speed and direction from knob reader
            current_speed = self.knob_reader.get_speed()
            current_direction = self.knob_reader.get_direction()
            
            # Update display if values changed
            if current_speed != self.last_speed or current_direction != self.last_direction:
                self.last_speed = current_speed
                self.last_direction = current_direction
                
                # Update labels
                self.speed_label.setText(f"{current_speed:.1f} km/hr")
                self.direction_label.setText(current_direction)
                ...
                self.popup_timer.start(2000)  # Hide after 2 seconds of no change
                
        except Exception as e:
            print(f"Error updating knob display: {e}")
    
    def show_popup(self):
        self.is_popup_visible = True
        # Animate button to small size
        self.button_animation.setStartValue(self.knob_button.geometry())
        self.button_animation.setEndValue(QtCore.QRect(70, 70, 60, 60))
        self.button_animation.start()
        
        # Animate popup to show
        self.popup_animation.setStartValue(QtCore.QRect(0, 0, 0, 0))
        self.popup_animation.setEndValue(QtCore.QRect(0, 0, 200, 200))
        self.popup_animation.start()
        self.knob_popup.show()
    
    def hide_popup(self):
        self.is_popup_visible = False
        # Animate popup to hide
        self.popup_animation.setStartValue(self.knob_popup.geometry())
        self.popup_animation.setEndValue(QtCore.QRect(0, 0, 0, 0))
        self.popup_animation.start()
        
        # Animate button back to center
        self.button_animation.setStartValue(self.knob_button.geometry())
        self.button_animation.setEndValue(QtCore.QRect(70, 70, 60, 60))
        self.button_animation.start()
        
        # Hide popup after animation
        QtCore.QTimer.singleShot(300, self.knob_popup.hide)
...
class VoltageSettingsDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        # Create radio buttons for all parameters
        self.radio_buttons = {}
        
        # Parameter configurations
        self.param_config = {
            'volt': {'label': 'Voltage (V)', 'range': (0, 220)},
            'current': {'label': 'Current (A)', 'range': (0, 3)},
            'hirz': {'label': 'Frequency (Hz)', 'range': (0, 60)},
            'power': {'label': 'Power (W)', 'range': (0, 500)},
            'thermal': {'label': 'Thermal Motor (%)', 'range': (0, 100)},
            'motor_factor': {'label': 'Motor Load (%)', 'range': (0, 100)}
        }
        
        # Create radio buttons
        for param, config in self.param_config.items():
            self.radio_buttons[param] = QtWidgets.QRadioButton(config['label'])
            group_layout.addWidget(self.radio_buttons[param])
        
        # Set default selection
        self.radio_buttons['volt'].setChecked(True)
    ...
    def get_selected_parameter(self):
        for param, button in self.radio_buttons.items():
            if button.isChecked():
                config = self.param_config[param]
                return param, config['label'].split(' ')[0], config['range']
        return 'volt', 'Voltage', (0, 500)  # Default
...
class MainWindow(QMainWindow, Ui_HMI):
    def __init__(self):
        ...
        # Initialize speedpopup
        # self.speed_popup.show()  # ??????????????? ?????????????????????????????        
        # Initialize current parameters for each gauge
        self.current_params = {
            'gauge1': 'volt',     # Default to voltage
            'gauge2': 'current',  # Default to current
            'gauge3': 'hirz',     # Default to frequency
            'gauge4': 'power'     # Default to power
        }
        
        # Parameter ranges and units
        self.param_config = {
            'volt': {'label': 'VOLTAGE', 'unit': 'V', 'range': (0, 220)},
            'current': {'label': 'CURRENT', 'unit': 'A', 'range': (0, 3)},
            'hirz': {'label': 'FREQUENCY', 'unit': 'Hz', 'range': (0, 60)},
            'power': {'label': 'POWER', 'unit': 'W', 'range': (0, 500)},
            'thermal': {'label': 'THERMAL MOTOR', 'unit': '%', 'range': (0, 100)},
            'motor_factor': {'label': 'MOTOR LOAD', 'unit': '%', 'range': (0, 100)}
        }

        # Add plot
        self.add_plot()

        # Setup update timer for real values
        self.update_timer = QTimer()
        self.update_timer.timeout.connect(self.update_gauge_values)
        self.update_timer.start(500)  # Update every 100ms
        ...
        self.blink_timer.timeout.connect(self.update_all_buttons)
    ...
    def handle_menu_click(self, index):
        ...
        # Ensure knob control stays on top
        self.knob_container.raise_()
        if self.is_popup_visible:
            self.knob_popup.raise_()
    ...
    def update_gauge_values(self):
        try:
            # Get real values from status()
            data = SD.read_status_vfd()
            
            # Map values to parameters
            value_dict = {
                'hirz': float(data[0]),      # Frequency
                'current': float(data[1]),    # Current
                'volt': float(data[2]),       # Voltage
                'power': float(data[3]),      # Power
                'thermal': float(data[4]),    # Thermal
                'motor_factor': float(data[5]) # Motor Load
            }
            # Update gauges based on their current parameter settings
           # self.ui.gauge_widget1.set_speed(value_dict[self.current_params['gauge1']])
           # self.ui.gauge_widget2.set_speed(value_dict[self.current_params['gauge2']])
           # self.ui.gauge_widget3.set_speed(value_dict[self.current_params['gauge3']])
           # self.ui.gauge_widget4.set_speed(value_dict[self.current_params['gauge4']])
           # print(f"parameter:{value_dict['hirz']}")

            for gauge_num in range(1, 5):
                gauge_key = f'gauge{gauge_num}'
                param = self.current_params[gauge_key]
                value = value_dict[param]
                
                gauge_widget = getattr(self.ui, f'gauge_widget{gauge_num}')
                gauge_widget.set_speed(value)
                gauge_widget.update()  # Force update
        except Exception as e:
            print(f"Error updating gauge values: {e}")
